    ...
    <div v-else-if="session">
      <div class="bg-[#1A2238] rounded-xl p-6 mb-6">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-6">
          <div>
            <h1 class="text-3xl font-bold text-white mb-2">{{ session.movie?.title }}</h1>
          ...
          <div class="text-right mt-4 md:mt-0">
            ...
            <div class="text-2xl font-bold text-[#0068C8]">{{ connectedUsers }}</div>
          </div>
        </div>

        <UAlert v-if="notification" :color="notificationType" variant="subtle" class="mb-6">
          {{ notification }}
        </UAlert>
        ...
        <div v-if="recentActivity.length > 0" class="mt-6 pt-6 border-t border-gray-700">
          <h3 class="text-sm font-semibold text-gray-400 mb-2">Activitat recent</h3>
          <div class="space-y-1">
            <div
      ...
      <div v-if="blockedTimeRemaining" class="bg-yellow-600 text-white px-4 py-2 rounded-lg text-center mb-4">
...
const proceedToCheckout = async () => {
  ...
  try {
    ...
    const errorMessage = error.data?.message || error.message || 'Error en bloquejar els seients'
