<template>
  <div class="container mx-auto px-4 py-8">
    ...
    <div v-if="!hasBlockedSeats" class="text-center py-16">
      <div class="text-gray-400 text-xl mb-4">No hi ha seients seleccionats</div>
      <NuxtLink to="/movies">
    ...
    <div v-else class="max-w-2xl mx-auto">
      <div class="bg-[#1A2238] rounded-xl p-6 mb-6">
        ...
        <div class="space-y-3 text-gray-300">
          <div class="flex justify-between">
            ...
            <span class="text-white font-medium">{{ session?.movie?.title }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-400">Cinema:</span>
            <span class="text-white font-medium">{{ session?.cinema_name }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-400">Sala:</span>
            <span class="text-white font-medium">{{ session?.room?.name }}</span>
          </div>
          <div class="flex justify-between">
            <span class="text-gray-400">Data i hora:</span>
            <span class="text-white font-medium">{{ formatDate(session?.start_time) }}</span>
          </div>
        </div>
      </div>
      ...
      <div v-if="expiresAt && !paymentSuccess" class="bg-yellow-600/20 border border-yellow-600 rounded-lg p-4 mb-6">
      ...
      <div v-if="paymentSuccess" class="mt-6 bg-green-600/20 border border-green-600 rounded-xl p-6 text-center">
        <UIcon name="i-heroicons-check-circle" class="w-16 h-16 text-green-500 mx-auto mb-4" />
        <p class="text-green-400 font-semibold text-xl">Pagament realitzat amb èxit!</p>
...
const priceItems = computed(() => {
  ...
  blockedSeats.value.forEach(seatId => {
    ...
    if (seat) {
      ...
      if (!seatPrices[key]) {
        seatPrices[key] = { count: 0, price: seat.type === 'vip' ? price.value * 1.5 : price.value }
      }
