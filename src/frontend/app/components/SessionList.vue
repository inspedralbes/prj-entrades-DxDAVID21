    ...
    >
      <div class="flex items-center gap-4">
        <div class="text-center px-4 border-r border-gray-600">
          <div class="text-[#F7C600] font-bold text-xl">{{ formatDate(session.date) }}</div>
          <div class="text-gray-400 text-sm">{{ formatTime(session.date) }}</div>
        </div>
        <div>
          <div class="text-white font-semibold">{{ session.cinema_name }}</div>
          <div class="text-gray-400 text-sm">Sala {{ session.room_number }}</div>
        </div>
      </div>
      <div class="flex items-center gap-4">
        <div class="text-right">
          <div class="text-[#0068C8] font-bold">{{ formatPrice(session.price) }}</div>
          <div class="text-gray-400 text-xs">{{ session.available_seats }} localitats</div>
        </div>
        <UButton
          color="primary"
          variant="solid"
